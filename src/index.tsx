...
import {
  ...
  NON_STEAM_ROUTE,
  HEROIC_ROUTE,
} from "./components/NonSteamGamesModal";
import { PlayNextPage, PLAY_NEXT_ROUTE } from "./components/PlayNextModal";
import {
  ExcludedGamesPage,
  EXCLUDED_GAMES_ROUTE,
} from "./components/ExcludedGamesModal";
...
import {
  SimilarToPageWrapper,
  SIMILAR_TO_ROUTE,
} from "./components/SimilarToPage";
import {
  LibraryBrowserPage,
  LIBRARY_BROWSER_ROUTE,
  FilterPoolPage,
  FILTER_POOL_ROUTE,
...
export default definePlugin(() => {
  ...
  routerHook.addRoute(NON_STEAM_ROUTE, () => (
    <ExternalGamesPage initialTab="nonsteam" />
  ));
  routerHook.addRoute(HEROIC_ROUTE, () => (
    <ExternalGamesPage initialTab="heroic" />
  ));
  ...
  routerHook.addRoute(FILTER_POOL_ROUTE, FilterPoolPage);

  try {
    const registration =
      SteamClient.GameSessions.RegisterForAppLifetimeNotifications((data) => {
        if (!data.bRunning && data.unAppID) {
          logger.info(
            `[SuggestMe] Game closed: ${data.unAppID}, syncing playtime...`,
          );
          call<[number], { success: boolean }>(
            "sync_playtime_for_app",
            data.unAppID,
  ...
  return {
    ...
    onDismount() {
      ...
      routerHook.removeRoute(HEROIC_ROUTE);
      ...
      routerHook.removeRoute(FILTER_POOL_ROUTE);
    },
  };
});
